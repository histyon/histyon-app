      ...
      <main className="flex-1 pt-20 pb-20 w-full max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center h-full">
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 flex flex-col justify-center">
            ...
            <p className="text-lg text-gray-500 leading-relaxed max-w-lg">{t.hero.description}</p>
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
              <Link href="/auth/register" className="inline-flex items-center justify-center gap-2 bg-black text-white px-8 py-3.5 rounded-lg font-medium text-base hover:bg-gray-800 transition-all hover:translate-y-[-1px] w-full sm:w-auto shadow-xl shadow-black/5">
                {t.hero.ctaAccess} <ArrowRight className="w-4 h-4" />
              </Link>
              <Link href="/documentation" className="inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-lg font-medium text-base text-gray-600 border border-gray-200 hover:bg-gray-50 transition-colors w-full sm:w-auto">
                {t.hero.ctaDocs}
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-8 border-t border-gray-100 mt-4">
               <div className="p-4 rounded-xl bg-gray-50 border border-gray-100/50">
                  ...
                  <h3 className="font-bold text-sm">{t.features.security.title}</h3>
                  <p className="text-xs text-gray-500 mt-1">{t.features.security.desc}</p>
               </div>
               <div className="p-4 rounded-xl bg-gray-50 border border-gray-100/50">
                  <Database className="w-5 h-5 text-black mb-2" />
                  <h3 className="font-bold text-sm">{t.features.storage.title}</h3>
                  <p className="text-xs text-gray-500 mt-1">{t.features.storage.desc}</p>
               </div>
               <div className="p-4 rounded-xl bg-gray-50 border border-gray-100/50">
                  <Microscope className="w-5 h-5 text-black mb-2" />
                  <h3 className="font-bold text-sm">{t.features.ai.title}</h3>
                  <p className="text-xs text-gray-500 mt-1">{t.features.ai.desc}</p>
               </div>
            </div>
          </div>
                ...
                <div className="flex-1 bg-gray-50 p-4 flex flex-col gap-4 overflow-hidden">
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <div className="bg-black text-white px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1.5 shadow-lg">
                           <LayoutDashboard className="w-3 h-3" /> {t.features.console}
                        </div>
                    </div>
